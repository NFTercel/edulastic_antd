...
describe('Test Assignment Page', () => {
  ...
  const buttonText = {'start':'START ASSIGNMENT',
                      'retake':'RETAKE',
                      'resume':'RESUME',
                      'review':'REVIEW' }
    ...
    it('1st attempt - test assessment player,attempt all questions and submit the test,validate', () => {
      ...
      ques.forEach((queNum, index, allQue) => {
        ...
        if (index == (total - 1)) {
          ...
          test.getNext()
            .should('not.have.class','ant-btn-icon-only');
        }
      })
      ...
      test.clickOnProceed();
      cy.url()
        .should('include', '/home/assignments');
  ...
  context('Assignment with start and due-date scenarios', () => {
    ...
    it('Verify assignments entry before and after due date', () => {
      ...
      cy.wait('@asgns');
      cy.wait('@summary');
      assignmentPage.getAssignmentButton()
        .should('not.be.visible');
    });
  })
});